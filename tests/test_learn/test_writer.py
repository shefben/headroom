...
class TestClaudeCodeWriter:
    ...
    def test_ccr_retrieve_literacy_section_preserves_other_sections(self, tmp_path):
        proj = _project(tmp_path)
        claude_md = proj.project_path / "CLAUDE.local.md"
        claude_md.write_text(
            f"{_MARKER_START}\n"
        ...
        assert "### CCR Retrieve Literacy" in content
        assert "Trust kept rows unless you have a concrete gap." in content
